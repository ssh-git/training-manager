$(function () {
    ...
    var COURSE_NAME_COLUMN_SELECTOR = '#course-name-column',
        ...
        MARKER_FOR_PLAN = { value: 'P', css: 'label-warning', title: 'Planned' },
        MARKER_FOR_START = { value: 'S', css: 'label-primary', title: 'Started' },
        MARKER_FOR_FINISH = { value: 'F', css: 'label-success', title: 'Finished' },
        ...
        onAllStatisticClick = function () {
            dtApi.column(7).visible(true);
            dtApi.column(8).visible(true);
        },

        onPlannedStatisticClick = function () {
            dtApi.column(7).visible(false);
            dtApi.column(8).visible(false);
        },

        onStartedStatisticClick = function () {
            dtApi.column(7).visible(true);
            dtApi.column(8).visible(false);
        },

        onFinishedStatisticClick = function () {
            dtApi.column(7).visible(false);
            dtApi.column(8).visible(true);
        },
        ...
        setStartState = function (row, date, dateOrdinal) {
            var dtStartDateCell = dtApi.cell(row, START_DATE_COLUMN_SELECTOR),
                controlsCell = dtApi.cell(row, CONTROL_COLUMN_SELECTOR).node(),
                markerNode = $('span', dtApi.cell(row, COURSE_NAME_COLUMN_SELECTOR).node());
        ...
        setFinishState = function (row, date, dateOrdinal) {
        ...
        onStartLearning = function (event) {
        ...
        onFinishLearning = function (event) {
        ...
        onRateCourse = function (event) {
            var rateControl = $(event.currentTarget),
                ...
                onSaveSuccessed = function (data, textStatus, jqXHR) {
                ...
                onSaveFailed = function (jqXHR, textStatus, errorThrown) {
                    dt.notifyError('Error occurred.');
                },

                onSaveChanges = function () {
                    var form = $('form', courseRateModal);
                    $.post(form.attr('action'), form.serialize(), onSaveSuccessed).error(onSaveFailed);
                };

            rateModalSaveButton.on('click', onSaveChanges);

            $('.modal-body', courseRateModal).load(url, function (responseText, textStatus, jqXhr) {
                var rateForm = $('form', courseRateModal);
                if (rateForm.length) {
                    $('.tm-datepicker', courseRateModal).datepicker({
                        constrainInput: true,
        ...
        onCourseRemoveModalHide = function () {
            removeModalDeleteButton.off();
        },

        onCourseRateModalHide = function () {
